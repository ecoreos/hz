...
class quick_record_search(models.Model):
            ...
            if current_user.quick_search_record_id :
                ...
                if current_user.quick_search_record_id.field_id.ttype == 'integer':
                    if record_no.isdigit():
                        ...
                        value.update({'msg_id':'No se encuentra ningun registro para %s'%(name)})
                else:
                    value=self.get_action(name,record_no, model, view_id,tree_view_id, field,search_type,window_type)
                print value
                return value
    ...
    def get_action(self,name,record_no, model, view_id,tree_view_id, field,search_type,window_type):
        ...
        domain=None
        if model == 'ir.ui.menu':
            search = '%'+record_no.upper()+'%'
            current_user = self.env['res.users'].browse(SUPERUSER_ID)
            # self.env.cr.execute("""
            #     select id from ir_ui_menu where UPPER(menu_name_search) like '%s'
            #     """ % search)
            self.env.cr.execute("""
                select ir_ui_menu.id from ir_ui_menu join ir_translation
                        on ir_ui_menu.name = ir_translation.src where UPPER(ir_translation.value) like %s and lang=%s;
                """,(search,current_user.lang))
            cr_res = self.env.cr.fetchall()
            record_ids = []
            if cr_res:
                record_cr_ids = [x[0] for x in cr_res]
                if record_cr_ids:
                    for menu in self.env['ir.ui.menu'].browse(record_cr_ids):
                        if menu.action:
                            if menu.action._name == 'ir.actions.act_window':
                                    record_ids.append(menu)
                self.env.cr.execute("""
        ...
        if record_ids:
            ...
            vals.update({'msg_id':'No se encuentra ningun registro para %s'%(name)})
            return vals     
